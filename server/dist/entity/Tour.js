"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.Tour = void 0;
const Review_1 = require("./Review");
const typeorm_1 = require("typeorm");
const type_graphql_1 = require("type-graphql");
let Tour = class Tour extends typeorm_1.BaseEntity {
};
__decorate([
    type_graphql_1.Field(() => type_graphql_1.Int),
    typeorm_1.PrimaryGeneratedColumn(),
    __metadata("design:type", Number)
], Tour.prototype, "id", void 0);
__decorate([
    type_graphql_1.Field(),
    typeorm_1.Column(),
    __metadata("design:type", String)
], Tour.prototype, "name", void 0);
__decorate([
    type_graphql_1.Field(() => [String]),
    typeorm_1.Column("text", { nullable: true }),
    __metadata("design:type", Array)
], Tour.prototype, "image", void 0);
__decorate([
    type_graphql_1.Field(),
    typeorm_1.Column(),
    __metadata("design:type", String)
], Tour.prototype, "description", void 0);
__decorate([
    type_graphql_1.Field(),
    typeorm_1.Column({ nullable: true }),
    __metadata("design:type", String)
], Tour.prototype, "location", void 0);
__decorate([
    type_graphql_1.Field(),
    typeorm_1.Column({ type: "numeric", default: 0.00 }),
    __metadata("design:type", Number)
], Tour.prototype, "price", void 0);
__decorate([
    typeorm_1.Column('int', { nullable: true }),
    type_graphql_1.Field(() => [Review_1.Review]),
    typeorm_1.OneToMany(type => Review_1.Review, review => review.tour, { cascade: ["insert"] }),
    __metadata("design:type", Array)
], Tour.prototype, "reviews", void 0);
Tour = __decorate([
    type_graphql_1.ObjectType(),
    typeorm_1.Entity()
], Tour);
exports.Tour = Tour;
